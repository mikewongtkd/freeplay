...
export const scenarioController = {
  async load(id) {
    ...
    switch (id) {
      ...
      case 'F': { const r = await reviewController.createRequest('chung', {origin: 'referee', issues: ['Last-five-seconds prohibited act']}); await reviewController.startReview(); playbackController.seekTo(r.rm - 2); await reviewController.markAUR(); await reviewController.setResult('accepted'); break; }
