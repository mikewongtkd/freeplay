...
export const reviewController = {
  ...
  async markAUR() { const review = activeReview() || selectedReview(); if (!review || !['active', 'completed'].includes(review.status)) return; return execute(async () => { const result = useResponse(await mockServer.markAur({ring: state.ring, reviewId: review.id, time: state.playbackCursor}), review.id); notify('review-updated'); return result; }); },
