...
function useResponse(response, selectedId) { replaceReviews(response.data.reviews, selectedId || response.data.review?.id); const review = selectedReview(); state.timelineRange.end = review?.rst || state.liveEdge; notify('review-response'); return response.data.review; }
...
export const reviewController = {
  ...
  async createRequest(side, options = {}) { return execute(async () => { const response = await mockServer.createRequest({ring: state.ring, side, origin: options.origin || 'coach', issueType: options.issueType || 'nontechnical', issues: options.issues || [], linkedReviewId: options.linkedReviewId || null}); const review = useResponse(response); state.timelineRange.end = review.rst || state.liveEdge; state.playbackCursor = review.rm; state.playbackState = 'paused'; notify('request-created'); return review; }); },
  ...
  selectReview(id, moveCursor = true) { if (!state.reviewHistory.some(review => review.id === id)) return; state.currentRequest = id; const review = selectedReview(); state.reviewWindow = {start: review.windowStart, end: review.windowEnd}; state.aur = review.aur; state.rst = review.rst; state.reviewStatus = review.status; state.reviewResult = review.result; state.timelineRange.end = review.rst || state.liveEdge; if (moveCursor) playbackController.seekTo(review.windowStart); notify('review-selected'); },
